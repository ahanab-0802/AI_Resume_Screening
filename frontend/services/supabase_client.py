...
def sign_up_with_password(email: str, password: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # dashboard "Site URL" (often left at its default, e.g. localhost:3000
        # or :5000) — the confirmation email then links somewhere this app
        # isn't running. Pointing it at our own configured URL fixes that.
...
def _humanize(exc: Exception) -> str:
    ...
    if 'unsupported provider' in msg.lower() or 'provider is not enabled' in msg.lower():
        return (
            'Google sign-in is not enabled on this Supabase project yet. '
            'In the Supabase dashboard: Authentication → Sign In / Providers → '
            'enable Google and add a Client ID/Secret, then try again.'
        )
    return msg
