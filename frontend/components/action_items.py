...
def _collect_action_items(analysis: Dict[str, Any]) -> List[Tuple[str, str, str]]:
    ...
    # Collapse repetitive "mention X in a project" suggestions into one concise action.
    grouped_skill_actions = []
    remaining = []
    for row in items:
        action = row[2]
        if row[1] == "Most Skills Lack Supporting Evidence" and action.lower().startswith("mention '"):
            grouped_skill_actions.append(action)
        else:
            remaining.append(row)

    if grouped_skill_actions:
        import re
        skills = []
        for action in grouped_skill_actions:
            m = re.search(r"Mention '([^']+)'", action, re.IGNORECASE)
            if m:
                skills.append(m.group(1))
        if skills:
            remaining.append(("medium", "Skill Evidence", "Add project or experience evidence for: " + ", ".join(skills)))

    remaining.sort(key=lambda row: SEVERITY_RANK.get(row[0], 99))
    return remaining
