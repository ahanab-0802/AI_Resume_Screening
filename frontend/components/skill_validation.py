...
def display_skill_validation(analysis: Dict[str, Any]) -> None:
    ...
    if unvalidated:
        with st.expander(f"⚠️ Skills without supporting evidence ({len(unvalidated)})", expanded=False):
            st.caption("These skills are listed on the resume but are not sufficiently demonstrated in a project or experience bullet.")
